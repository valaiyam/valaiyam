...
async function setStatus(projectId: string, status: string, extra: object = {}) {
  await supabase
    .from('projects')
    .update({ status, current_agent: status, ...extra })
    .eq('id', projectId)
}
...
async function logError(projectId: string, agent: string, error: string) {
  ...
  await supabase
    .from('projects')
    .update({ error_log: [...log, { agent, error, timestamp: new Date().toISOString() }] })
    .eq('id', projectId)
}
...
async function runPipeline(project: any) {
  ...
  try {
    ...
    await supabase
      .from('projects')
      .update({ generated_files: generatedFiles, retry_count: retries })
      .eq('id', projectId)
    ...
    const { previewUrl, vercelProjectId, githubRepo } = await runDeploymentAgent(
      projectId,
      generatedFiles,
      intake_data
    )
...
async function notifyCustomerPreview(project: any, previewUrl: string) {
  ...
  const emailText = [
    ...
    '- Or "Approved" if you are happy with it',
    '',
    'Once approved, we will send you the payment link (Rs. 4,999) and make your site live.',
    '',
    '-- Valaiyam Team',
  ].join('\n')
